...
describe('Image', () => {
  ...
  describe('STL', () => {
    // Alas, due to the way ImageMagick works, calling all container
    // functions with identical images such as [ src, src ]
    // results in an infinite loop inside the library
    // I will probably implement a check at some point
    it('mosaicImages', () => {
      const src1 = new Image(path.join(__dirname, 'data', 'wizard.png'));
      const src2 = new Image(src1.size(), new Color('red'));

      const dst = new Image();
      ImageMagick.Magick.averageImages(dst, [src1, src2]);
